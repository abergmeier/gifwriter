...
func (e *encoder) writeImageBlock(pm *image.Paletted, delay int, disposal byte, comment []byte) {
	if e.err != nil {
		return
	}
	...
	e.writeCommentExtension(comment)
...
func (e *encoder) writeCommentExtension(comment []byte) {
	e.buf[0] = sExtension
	e.buf[1] = eComment
	e.write(e.buf[:2])
	...
	for i := 0; i < len(comment); {
		...
		written := copy(e.buf[1:], comment[i:])
		e.buf[0] = byte(written)
		e.write(e.buf[:written+1])
...
func EncodeAll(w io.Writer, g *GIF) error {
	...
	for i, pm := range g.Image {
		...
		e.writeImageBlock(pm, g.Delay[i], disposal, comment)
	}
